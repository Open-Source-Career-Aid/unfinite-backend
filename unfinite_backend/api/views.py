...
def query(request):
    ...
    response = requests.post(settings.QUERYHANDLER_URL, headers={'Authorization':settings.QUERYHANDLER_KEY}, json={'query_text': query_text, 'user_id': request.user.id})
...
def search(request):
    ...
    response = requests.post(settings.QUERYHANDLER_URL, headers={'Authorization':settings.QUERYHANDLER_KEY}, json=data)
