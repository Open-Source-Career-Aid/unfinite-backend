...
def get_maxheading(parsedpdf):
    
    maxheight = 0
    for x in parsedpdf:
        if x['tag'][0:2]=='<h':
            if int(x['tag'][2:len(x['tag'])-1])>maxheight:
                maxheight = int(x['tag'][2:len(x['tag'])-1])
    return maxheight

def get_maxspan(parsedpdf):
    
    maxspan = 0
    for x in parsedpdf:
        if x['tag'][0:2]=='<s':
            if int(x['tag'][2:len(x['tag'])-1])>maxspan:
                maxspan = int(x['tag'][2:len(x['tag'])-1])
    return maxspan

def get_maxwidth(parsedpdf):
    
    maxwidth = 0
    for x in parsedpdf:
        if x['p_position_x']>maxwidth:
            maxwidth = x['p_position_x']
    return maxwidth

def get_maxheight(parsedpdf):
    
    maxheight = 0
    for x in parsedpdf:
        if x['p_position_y']>maxheight:
            maxheight = x['p_position_y']
    return maxheight

def get_elements_with_tag(parsedpdf, tag):
...
#     print(parsedpdf)
    ...
    for x in parsedpdf:
        if x['tag']==tag:
            listofelements.append(x['text'])
    return listofelements

def get_elements_under_and_before_the_next_heading_with_same_tag(parsedpdf, tag, text):
    
    listofelements = []
    cont = False
    
    for x in parsedpdf:
        if x['tag']==tag and x['text']==text:
            cont = True
            continue
        if cont:
            if x['tag']==tag:
...
def find_element_by_text(parsedpdf, text):
    
    for x in parsedpdf:
        if x['text']==text:
            return x

def find_title(parsedpdf):
    
    heightnow = get_maxheading(parsedpdf)
    maxwidth = get_maxwidth(parsedpdf)
#     print(maxwidth)
    maxheight = get_maxheight(parsedpdf)
#     print(maxheight)
    layerone = 0
    layertwo = 0
    while True:
        for element in parsedpdf:
            if element['tag']==f'<h{heightnow}>':
#                 print(int(element['p_position_y'])/maxheight)
                if element['page_num']==1 and int(element['p_position_y'])/maxheight<0.5: # title should be in the upper half of the first page
                    if layerone!=0:
                        continue
...
def find_outline(parsedpdf, titletag):
    
    # outline should contain 'abstract' or 'introduction' or 'references'
    heightnow = int(titletag[2:len(titletag)-1])
    
    while True:
        heightnow-=1
        totalelements = get_elements_with_tag(parsedpdf, f'<h{heightnow}>')
        for element in totalelements:
            if 'abstract' in element.lower() or 'introduction' in element.lower() or 'references' in element.lower():
                return f'<h{heightnow}>', totalelements
        if heightnow==0:
            break
    return None

def title_from_pdf(url):
    
    parsedpdf = pdf_to_dict_from_url(url)
