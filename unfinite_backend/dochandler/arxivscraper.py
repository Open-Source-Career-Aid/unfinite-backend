        ...
        if response.status_code == 200:
            ...
            if results:
                for index, result in enumerate(results, start=1):
                    ...
                    result_map['publisher'] = result.find("div", class_="gs_a").text.split("-")[-1].strip()
                    # print(result_map)
                    # break
                    result_map['authors'] = []
                    for item in result.find_all('div', {'class': 'gs_a'}):
                        authors = item.find_all('a')
                        for author in authors:
                            result_map['authors'].append(author.text)
                    # print(result_map)
                    # break
...
# if __name__ == "__main__":
    ...
    # google_scholar_scrape("vaswani", 1)
