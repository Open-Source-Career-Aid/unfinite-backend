...
def contentfinder_noJS(url):
    ...
    try:
        ...
        if not article:
            article = soup.find("main")
        description, para = soup.find('meta', attrs={'name': 'description'}), article
        description = description.get("content", "") if description else para.text[
                                                                         : min(len(para.text), 200)] if para else ""
...
def getpagetext(url):
    ...
    if article is None: return '', content_meta
    for element in article.find_all():
        # Check if the element is part of the readable article content and has a valid tag name
        if element.name in valid_tags:
            # Print the name of the element and its text content
...
def pooled_scrape(url):
    ...
    if len(pagedata)>100: # random number, but if the text is too short, it's probably not useful
        ...
        _metadata.append([str(url_meta['title']), url_meta['source'], str(url_meta['summary']),
                          str(url_meta['content_read_time']), str(url_meta['url'])])
